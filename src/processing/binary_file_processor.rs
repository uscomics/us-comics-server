...
pub fn binary_file_processor(preprocessing_response: &PreprocessingResponse) -> Result<ProcessingResponse, server_status::ServerStatus> {
    ...
    let response = ProcessingResponse::new(&server_status::OK, &mime::BINARY, &headers, &None, &preprocessing_response);
    Ok(response)
}
...
mod test {
    ...
    use crate::processing::binary_file_processor::*;
    use crate::util::server_status;
    use crate::util::mime;
    use crate::config::*;

    #[test]
    fn test_binary_file_processor() {
        let response_info = ResponseInfo::new(BINARY_FILE, None, Some("./path/to/:file".to_string()));
        ...
        let response = binary_file_processor(&preprocessor_response);
        match response {
            Ok(r) => {
