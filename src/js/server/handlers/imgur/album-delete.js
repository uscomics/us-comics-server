    ...
    return async (req, res, next) => {
        ...
        try {
            ...
            const response = await axios({
                ...
                url: `${imgurBaseURL}album/${req.params['id']}`,
                redirect: `follow`,
                data: formData
            })

            if (200 !== response.status) {
                const err = `Request to delete an album for user ${userName} failed. Status ${response.status}`
            ...
            const err = `Error deleting Imgur album. ${e.message}`
