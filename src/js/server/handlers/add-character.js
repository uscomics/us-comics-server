...
module.exports = (entry) => {
    return async (req, res, next) => {
        ...
        const finalDir = `./src/images/promo`
        ...
        form.parse(req, async (parseError, parseFields, parseFiles) => {
            ...
            try {
                ...
                const newFileName = `promo${newId}${newFileExtension}`
                const newCharacterRecord = { title: parseFields.title, file: newFileName }
                const createResult = await surrealDBCreate(db, newRecordId, newCharacterRecord)
